<template>
    <Teleport :to="body">
        <Transition name="modal-outer">
            <div v-show="modalActive" class="absolute w-full bg-black bg-opacity-30 h-screen top-0 left-0 flex justify px-8">
...
.modal-outer-enter-active , .modal-outer-leave-active{
    transition: opacity 0.3s cubic-bezier(0.52, 0.02, 0.19, 1.05);
}
.modal-outer-enter-from , .modal-outer-leave-to{
    opacity: 0;
}
.modal-inner-enter-active {
    transition: all 0.3s cubic-bezier(0.52, 0.02, 0.19, 1.05) 0.15s;
}
 .modal-inner-leave-active{
    transition: all 0.3s cubic-bezier(0.52, 0.02, 0.19, 1.05);
 }
